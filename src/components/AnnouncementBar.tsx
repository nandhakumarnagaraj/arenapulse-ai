...
export default function AnnouncementBar({ latestAnnouncement }: AnnouncementBarProps) {
  ...
  return (
    <div className="bg-gradient-to-r from-indigo-900/30 to-purple-900/30 rounded-xl border border-indigo-500/20 overflow-hidden" role="region" aria-label="Public announcements" aria-live="polite">
      <div className="px-4 py-3 border-b border-indigo-500/20 flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-lg" aria-hidden="true">📢</span>
          <h3 className="text-sm font-semibold text-white">Public Announcements</h3>
        </div>
        <div className="flex gap-1" role="tablist" aria-label="Announcement language">
          {languages.map((lang) => (
            <button
              key={lang.code}
              ...
              aria-label={`Show announcement in ${lang.label}`}
      ...
      <div className="p-4" role="tabpanel" aria-label={`Announcement in ${activeLang === "en" ? "English" : activeLang === "es" ? "Spanish" : "French"}`}>
